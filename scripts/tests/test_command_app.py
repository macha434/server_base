...
def test_app_add_passes_through_repo_url_only():
    with patch("server_base_cli.commands.app.shell.run_script") as mock_run:
        mock_run.return_value = 0
        parser = build_parser()
        args = parser.parse_args(
            ["cli", "app", "add", "https://github.com/coresync-fukuhara/time-announcement-frontend"]
        )
        code = args.func(args)

    mock_run.assert_called_once_with(
        "new-app.sh",
        ["https://github.com/coresync-fukuhara/time-announcement-frontend"],
    )
    assert code == 0


def test_app_add_forwards_optional_flags_when_given():
    with patch("server_base_cli.commands.app.shell.run_script") as mock_run:
        mock_run.return_value = 0
        parser = build_parser()
        args = parser.parse_args(
            [
                "cli", "app", "add", "https://github.com/org/myapp",
                "--service", "web",
